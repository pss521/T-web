var selectreturnlistFun = function () {
    layui.use(['laytpl', 'form','element','layer'], function () {
        ...
        var getTpl = selectreturnlistTmp.innerHTML
        ...
        $.post("/inventory", "", function (data) {
            ...
            product.forEach(function (prod, index) {
                ...
                html += '<i data-name="' + prod.Name + '" data-specification="' + prod.Specification + '" data-code="' + prod.Code + '" style="margin-left:10px;font-size:20px;" class="layui-icon layui-icon-add-circle-fine addreturn"></i>';
                html += '</p>';
                html += '</div>';
                html += '</div>';
                ...
                if (product.length - 1 === index) {
                    ...
                    $.post('/currentStockQuery',{parameter:JSON.stringify({param:{warehouse:[{code:sessionStorage['OutWarehouse']}]}})},function(currentStockQueryData){
        ...
        $("#addReturnRequest_detail").on("click", function () {
            location.hash = this.id;
            pages();
        });
