var saleslist_goodsFun = function () {
    layui.use(['laytpl', 'form','layer'], function () {
        ...
        var warehouse=sessionStorage['warehouse'];
        var getTpl = saleslist_goodsTmp.innerHTML
        ...
        $.get("/warehouse", function (data) {
            ...
            $('#warehouse').val(sessionStorage['warehouse']);
        })
        ...
        var one_price = JSON.parse(sessionStorage['addproduct']);
        $("#product_num").text(one_price.length);
        var total_num = 0;
        one_price.forEach(function (price, index) {
            ...
            html += '<p>单价：<input type="number" style="width:72px;" id="goodsmoney_' + (price.Code + index) + '"></p>'
            html += '<p>金额：<span id="moneygoods_' + (price.Code + index) + '"></span></p>'
            html += '</div>'
            html += '</div>'
            html += '<div class="cell-right" style="display:block;margin-left: -1000px;">'
            html += '<div style="line-height:72px">'
            html += '<p>数量：<input type="number" style="width:72px;" value="' + price.num + '"> 瓶</p>'
            html += '</div>'
            html += '</div>'
            html += '</div>'
            html += '</div>'
        ...
        $('.delete-swipeout').on("click",function(){
            ...
            layer.confirm('您确认删除次存货?', {icon: 3, title:false}, function(index){
                ...
                sessionStorage.setItem("addproduct",JSON.stringify(one_price))
                location.reload()
                layer.close(index);
              });
        });
        ...
        //取价
        $("#qujia").on("click", function () {
            var total_money = 0.00;
            one_price.forEach(function (val, key) {
                $("#goodsmoney_" + val.Code + key).val(val.DiscountPrice);
                $("#moneygoods_" + val.Code + key).text(val.DiscountPrice * val.num);
            ...
            layer.tab({
                ...
                tab: [{
                  ...
                  content:  '<div class="m-cell">'+
                            ...
                            '        <div class="cell-right"><input type="number" id="total_money1" pattern="[0-9]*" class="cell-input" value="" /></div>'+
                            '   </div>'+
                            '    <div class="cell-item">'+
                            ...
                            '            <select class="cell-select" id="SettleStyle1">'+
                            '            </select>'+
                            '        </label>'+
                            '    </div>'+
                            '    <div class="cell-item">'+
                            ...
                            '        <label class="cell-right cell-arrow">'+
                            '            <select class="cell-select" id="BankAccount1">'+
                            '            </select>'+
                            '        </label>'+
                            '    </div>'+
                            '    <div class="cell-item">'+
                            '        <div class="cell-left">现结：</div>'+
                            '        <div class="cell-right"><input type="number" pattern="[0-9]*" class="cell-input" id="xianjieSettleStyle1" placeholder="请输入" autocomplete="off" /></div>'+
                            '    </div>'+
                            '</div>'
                }, {
                  title: '赊账', 
                  ...
                  '    <div class="cell-item">'+
                  '        <div class="cell-left">总金额：</div>'+
                  '        <div class="cell-right"><input type="number" id="total_money2" pattern="[0-9]*" class="cell-input" value="" /></div>'+
                  '   </div>'+
                  '    <div class="cell-item">'+
                  '        <div class="cell-left">抹零：</div>'+
                  '        <div class="cell-right"><input type="number" pattern="[0-9]*" class="cell-input" placeholder="请输入" autocomplete="off" /></div>'+
                  ...
                  '        <div class="cell-left">结算方式：</div>'+      
                  '        <label class="cell-right cell-arrow">'+
                  '            <select class="cell-select" id="SettleStyle">'+
                  '            </select>'+
                  '        </label>'+
                  '    </div>'+
                  '    <div class="cell-item">'+
                  '        <div class="cell-left">账号：</div>'+
                  '        <label class="cell-right cell-arrow">'+
                  '            <select class="cell-select" id="BankAccount">'+
                  '            </select>'+
                  '        </label>'+
                  '    </div>'+
                  '    <div class="cell-item">'+
                ...
                success:function(){
                    ...
                    $("#total_money1").val($("#total_money").text());
                    $("#total_money2").val($("#total_money").text());
                    ...
                    //         $("#SettleStyle").append('<option value="'+val.Code+'">'+val.Name+'</option>');
                    //         $("#SettleStyle1").append('<option value="'+val.Code+'">'+val.Name+'</option>');
                    //     });
                    // });
                    ...
                    $.post("/BankAccount",function(data){
                        ...
                        data.data.forEach(function(val){
                            $("#BankAccount").append('<option value="'+val.Code+'">'+val.Name+'</option>');
                            $("#BankAccount1").append('<option value="'+val.Code+'">'+val.Name+'</option>');
                        });
                    });
                    $("#SettleStyle").append('<option value="0">和包支付卷</option>');
                    $("#SettleStyle1").append('<option value="0">和包支付卷</option>');
                    ...
                    for (var i = 0; i < one_price.length; i++) {
                        SaleDeliveryDetails.push({
                            ...
                            quantity: one_price[i].num,
                            origPrice: one_price[i].DiscountPrice
                        });
                    }
                    var jsondata = {
                        externalCode: "1",
                        customer: { code: $("#customer_list").val() },
                        invoiceType: { code: "02" },
                        warehouse:{code: $("#warehouse").val()},
                        memo: $("#Memo").val(),
                        project: { code: $("#project_list").val()},
                        saleDeliveryDetails: SaleDeliveryDetails,
                        origSettleAmount:$("#total_money").text()
                    ...
                    //     jsondata.saleDeliverySettlements[0].origAmount = $("#total_money1").val();
                    //     jsondata.saleDeliverySettlements[0].settleStyle.code = $("#SettleStyle1").val();
                    //     jsondata.saleDeliverySettlements[0].bankAccount.name = $("#BankAccount1").val();
                    // }else{
                    //     jsondata.saleDeliverySettlements[0].origAmount = $("#total_money2").val();
                    //     jsondata.saleDeliverySettlements[0].settleStyle.code = $("#SettleStyle").val();
                    //     jsondata.saleDeliverySettlements[0].bankAccount.name = $("#BankAccount").val()
                    // }
                    ...
                    $.post("/saleDeliveryCreate", {parameter:JSON.stringify(jsondata)}, function (data) {
                        ...
                        if(data.data == null){
                            ...
                            var zongjine = "";
                            if($(".layui-layer-title .layui-this").text() === "现结"){
                                zongjine = $("#total_money1").val();
                            }else{
                                zongjine =  $("#total_money2").val();
                            }
                            layer.msg('保存成功，总金额：'+zongjine+"元", function(){
                                //do something
